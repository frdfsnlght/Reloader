...
class BulletCollator(RelativeLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.blinkOn = True
        self.emptyTimeout = config.getint('core', 'bulletCollatorEmptyTimeout')
        self.sensorPort = config.getint('core', 'bulletSensorPort')
        self.motorPort = config.getint('core', 'bulletMotorPort')
        self.motorFrequency = config.getint('core', 'bulletMotorFrequency')
        ...
        self.update()
        bus.emit('bulletCollator/count', self.count, True)
    ...
    def setupGPIO(self):
        ...
        def cb(port, level, tick):
            ...
            if level == 1:
                ...
                bus.emit('bulletCollator/count', self.count, False)
    ...
    def start_motor(self):
        ...
        self.collatorEmptyTimer()
        self.stop_alert()
        bus.emit('bulletCollator/running')
        
    def stop_motor(self):
        pi.set_PWM_dutycycle(self.motorPort, 0)
        self.running = False
        self.collatorEmptyTimer.cancel()
        bus.emit('bulletCollator/stopped')
    ...
    def on_press_minus(self):
        ...
        self.stop_alert()
        self.update()
        bus.emit('bulletCounter/count', self.count, True)
    ...
    def on_press_settings(self):
        ...
        if not self.motorSpeedDialog:
            ...
            self.motorSpeedDialog.title = 'Bullet Motor'
