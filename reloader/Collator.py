...
class Collator(RelativeLayout):
    ...
    def on_long_press_playPause(self):
        self.stop_alert()
        MotorSpeedDialog(title = self.type.capitalize() + ' Motor',
                         value = self.motorDutycycle,
                         on_value = self.on_change_motorSpeed,
                         on_close = self.on_close_motorSpeed)
                         
    def on_change_motorSpeed(self, value):
        self.motorDutycycle = int(value)
        if self.running:
            pi.set_PWM_dutycycle(self.motorPort, self.motorDutycycle)

    def on_close_motorSpeed(self):
        Settings.settings().safe_set('collators', self.type + 'MotorSpeed', self.motorDutycycle)
